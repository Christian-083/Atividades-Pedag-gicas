...
function LazySection({ children, id }: { children: React.ReactNode, id?: string }) {
  ...
  React.useEffect(() => {
    // Dispara rápido no mobile (Lighthouse) para renderizar apenas a primeira dobra instantaneamente
    const isMobile = window.innerWidth < 768;
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting || entry.boundingClientRect.top < window.innerHeight * (isMobile ? 1.5 : 3)) {
          setIsVisible(true);
          observer.disconnect();
        }
      },
    ...
    // Fallback global de segurança para evitar conteúdo preso em caso de lentidão da api scroll no mobile
  ...
  return (
    <div ref={ref} id={id} style={{ contentVisibility: isVisible ? 'visible' : 'hidden', minHeight: isVisible ? 'auto' : '800px' }}>
...
export default function App() {
  ...
  return (
    ...
    >
      ...
      {/* ── PLANOS ── */}
      <LazySection id="pricing-plans-section">
      <section className="bg-[#1a3bb5] py-16 px-4 w-full">
        <div className="max-w-5xl mx-auto space-y-10">
